#!/usr/bin/env python3
"""
Kaggle Notebook: AI News Qwen Processor

This is a self-contained script for running on Kaggle.
Upload this as a Kaggle notebook and configure the secrets.
...
3. Set up Kaggle Secrets:
   - TELEGRAM_BOT_TOKEN
   - TELEGRAM_CHAT_ID
4. Upload request_queue.json as a Dataset or mount Google Drive
5. Run the notebook
...
# !pip install transformers>=4.35.0 accelerate requests
...
# Model configuration
MODEL_NAME = "Qwen/Qwen2.5-14B-Instruct"  # Change to 32B if needed
QUEUE_FILE = "request_queue.json"  # Update path as needed
POLL_INTERVAL = 30  # seconds

print(f"Model: {MODEL_NAME}")
print(f"Queue file: {QUEUE_FILE}")

# Cell 4: Queue Manager
class QueueManager:
    ...
    try:
        ...
        if pending:
            ...
            for request in pending:
                ...
                print(f"  - {request_type} for: {article_title}")
                ...
                try:
                    ...
                    chat_id = request.get("chat_id")
                    if chat_id:
                        telegram.send(f"❌ Failed: {request_type} for {article_title}\n\nError: {e}")
        else:
